...
Key CLI flags: `--client`, `--model`, `--api-key`, `--api-url`, `--whisper-model`, `--max-frames`, `--start-stage` (1-3), `--log-level`, `--prompt`, `--output`.
...
Default output: `output/analysis.json`. Frames are cleaned up unless `--keep-frames` is set.
...
1. Run `video-analyzer <video> --keep-frames` to produce `output/analysis.json` + `output/frames/`
...
- Frame analysis uses `num_predict=300`; reconstruction uses `num_predict=1000` (from `response_length` config)
